...
[實驗與評估 - 能源效率分析框架](#實驗與評估-能源效率分析框架) -> 為了比較Data Flow效果的模擬設定，作者甚至做了一個通用性的最佳傳輸排程邏輯，可以用於所有數據流  
...
為了比較Data Flow效果的模擬設定，作者甚至做了一個通用性的最佳傳輸排程邏輯，可以用於所有數據流

在每一次 MAC 運算（對應於公式 (1)）中，從記憶體提取輸入（ifmap 與 filter）並進行 psum 累加，會因以下兩點而產生不同的能源成本：
- 資料流如何實現資料重用與 psum 累加的排程（詳見第 III-B 節）
- 從不同儲存層級存取資料的能源成本不同

能源效率高的 CNN 資料流設計，其目標是：

- 儘量使用 能源成本較低的資料移動通道 來完成資料存取。
- 最佳化過程，會依據：資料存取總量、CNN 層的形狀、可用的硬體資源，而有所不同。

框架功能：

本節提出的分析框架可作為一套工具，專門用來針對空間架構中的資料流進行優化。其功能如下：
- 為儲存階層中每一層（如 RF、PE 陣列、global buffer）定義對應的能源成本；
- 提供簡單方法，將任意資料流納入這個階層模型中，量化其整體資料移動所消耗的能量；
- 可根據 CNN 層形狀與硬體配置，自動尋找該資料流下的最佳映射策略，以達到最優能源效率。
